 ...
 * @author Sergey
 */
public class Lesson09Part1List {
...
// ≈ “один шаг”; размер почти не влияет | ≈ “пройтись по всем”; рост прямо пропорционален
    ...
    /*
    List — необходимый набор вариантов создания (популярные случаи)

    1) ArrayList (динамический массив) — базовый выбор “по умолчанию”
    Создание пустого списка:
    java.util.List<Тип> имяСписка = new java.util.ArrayList<>();

    Создание с ожидаемой вместимостью (если заранее известен примерный размер):
    ...
    java.util.List<Тип> имяСписка = new java.util.ArrayList<>(sourceCollection);

    Неизменяемый список (константа/фиксированный набор значений):
    java.util.List<Тип> имяСписка = java.util.List.of(элемент1, элемент2, ...);

    Неизменяемая копия (immutable snapshot) существующей коллекции:
    java.util.List<Тип> имяСписка = java.util.List.copyOf(sourceCollection);

    “Только чтение” как представление (view) над изменяемым списком:
    java.util.List<Тип> имяСписка = java.util.Collections.unmodifiableList(mutableList);
     */
